            ...
            {empApplication.map((app) => (
              <tr key={app.id}>
                ...
                <td className='bg-info'>
                  <button className='bg-info fw-bold' style={{ border: 'none', color: 'red' }} onClick={() => handleCancel(app.id, app.status)}>Cancel</button>
                </td>
                <td>
                  <button className='p-1 bg-success' onClick={() => handleViewDetails(app)}>View</button>
                </td>
              </tr>
            ))}
          </tbody>
